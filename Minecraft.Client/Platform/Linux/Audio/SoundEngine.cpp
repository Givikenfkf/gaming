...
FAudio* g_pXAudio2 = NULL;       // pointer to XAudio2 instance used by QNet and XACT
FAudioMasteringVoice* g_pXAudio2MasteringVoice = NULL;  // pointer to XAudio2 mastering voice
...
F3DAUDIO_DSP_SETTINGS	SoundEngine::m_DSPSettings;
F3DAUDIO_EMITTER		SoundEngine::m_emitter;
F3DAUDIO_LISTENER		SoundEngine::m_listeners[4];
int						SoundEngine::m_validListenerCount = 0;

F3DAUDIO_DISTANCE_CURVE_POINT SoundEngine::m_VolumeCurvePoints[2] = {
	{0.0f, 1.0f},
	{1.0f, 0.0f},
};

F3DAUDIO_DISTANCE_CURVE_POINT SoundEngine::m_DragonVolumeCurvePoints[2] = {
	{0.0f, 1.0f},
	{1.0f, 0.5f},
};
F3DAUDIO_DISTANCE_CURVE_POINT SoundEngine::m_VolumeCurvePointsNoDecay[2] = {
	{0.0f, 1.0f},
	{1.0f, 1.0f},
};

F3DAUDIO_DISTANCE_CURVE	SoundEngine::m_VolumeCurve;
F3DAUDIO_DISTANCE_CURVE	SoundEngine::m_DragonVolumeCurve;
F3DAUDIO_DISTANCE_CURVE	SoundEngine::m_VolumeCurveNoDecay;

void SoundEngine::setXACTEngine( FACTAudioEngine *pXACT3Engine)
{
...
void SoundEngine::init(Options *pOptions)
{
	...
	HRESULT hr;
	if ( FAILED ( hr = FACTCreateEngine( 0, &m_pXACT3Engine ) ) )
	{
		app.FatalLoadError();
		assert( false );
		return;
	...
	HANDLE file;
#ifdef _TU_BUILD
	file = CreateFile("Common/res/audio/Minecraft.xgs", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
	file = CreateFile("Common/res/TitleUpdate/audio/Minecraft.xgs", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
	...
	DWORD bytesRead = 0;
	void *pvGlobalSettings = malloc(dwFileSize);
	ReadFile(file,pvGlobalSettings,dwFileSize,&bytesRead,NULL);
	CloseHandle(file);

	FACTRuntimeParameters EngineParameters = {0};
	EngineParameters.lookAheadTime = FACT_ENGINE_LOOKAHEAD_DEFAULT;
	...
	EngineParameters.globalSettingsFlags = FACT_FLAG_MANAGEDATA;
	EngineParameters.pXAudio2 = g_pXAudio2;
	EngineParameters.pMasteringVoice = g_pXAudio2MasteringVoice;

	if ( FAILED ( hr = FACTAudioEngine_Initialize( m_pXACT3Engine, &EngineParameters ) ) )
	{
		app.FatalLoadError();
		assert( false );
		return;
	...
	// Initialise X3D
	FACT3DInitialize(m_pXACT3Engine,m_xact3dInstance);

	// Set up common structures that can be re-used between sounds & just have required bits updated
	memset(&m_DSPSettings,0,sizeof(F3DAUDIO_DSP_SETTINGS));
	FAudioWaveFormatExtensible format;
	FACTAudioEngine_GetFinalMixFormat(m_pXACT3Engine, &format);
	...
	for( int i = 0; i < 4; i++ )
	{
		memset(&m_listeners[i],0,sizeof(F3DAUDIO_LISTENER));
		m_listeners[i].OrientFront.z = 1.0f;
		m_listeners[i].OrientTop.y = 1.0f;
	}
	m_validListenerCount = 1;
	memset(&m_emitter,0,sizeof(F3DAUDIO_EMITTER));
	...
	file = CreateFile("Common/res/audio/resident.xwb", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if( file == INVALID_HANDLE_VALUE )
	{
		app.FatalLoadError();
		assert(false);
	...
	void *pvWaveBank = malloc(dwFileSize);
	ReadFile(file,pvWaveBank,dwFileSize,&bytesRead,NULL);
	CloseHandle(file);

	if ( FAILED( hr = FACTAudioEngine_CreateInMemoryWaveBank( m_pXACT3Engine, pvWaveBank, dwFileSize, FACT_FLAG_MANAGEDATA, 0, &m_pWaveBank ) ) )
	{
		app.FatalLoadError();
		assert(false);
		return;
...
#ifdef _TU_BUILD
	file = CreateFile("Common/res/audio/additional.xwb", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
	file = CreateFile("Common/res/TitleUpdate/audio/additional.xwb", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
	if( file == INVALID_HANDLE_VALUE )
	{
		app.FatalLoadError();
	...
	dwFileSize = GetFileSize(file,NULL);
	void *pvWaveBank2 = malloc(dwFileSize);
	ReadFile(file,pvWaveBank2,dwFileSize,&bytesRead,NULL);
	CloseHandle(file);

	if ( FAILED( hr = FACTAudioEngine_CreateInMemoryWaveBank( m_pXACT3Engine, pvWaveBank2, dwFileSize, FACT_FLAG_MANAGEDATA, 0, &m_pWaveBank2 ) ) )
	{
		app.FatalLoadError();
		assert(false);
		return;
	...
	FAudioIOStream *faudioFile = NULL;
	faudioFile = FAudio_fopen("Common/res/audio/streamed.xwb");
	...
	FACTStreamingParameters streamParams;
	streamParams.file = faudioFile;
	...
	if ( FAILED( hr = FACTAudioEngine_CreateStreamingWaveBank( m_pXACT3Engine, &streamParams, &m_pStreamedWaveBank ) ) )
	{
		app.FatalLoadError();
		assert(false);
		return;
...
#ifdef _TU_BUILD
	file = FAudio_fopen("Common/res/audio/AdditionalMusic.xwb");
#else
	faudioFile = FAudio_fopen("Common/res/TitleUpdate/audio/AdditionalMusic.xwb");
#endif
	if( faudioFile == NULL )
	{
		app.FatalLoadError();
	...
	if ( FAILED( hr = FACTAudioEngine_CreateStreamingWaveBank( m_pXACT3Engine, &streamParams, &m_pStreamedWaveBankAdditional ) ) )
	{
		app.FatalLoadError();
		assert(false);
		return;
...
#ifdef _TU_BUILD
	file = CreateFile("Common/res/audio/minecraft.xsb", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
	file = CreateFile("Common/res/TitleUpdate/audio/minecraft.xsb", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
	if( file == INVALID_HANDLE_VALUE )
	{
		app.FatalLoadError();
	...
	dwFileSize = GetFileSize(file,NULL);
	void *pvSoundBank = malloc(dwFileSize);
	ReadFile(file,pvSoundBank,dwFileSize,&bytesRead,NULL);
	CloseHandle(file);

	if ( FAILED( hr = FACTAudioEngine_CreateSoundBank( m_pXACT3Engine, pvSoundBank, dwFileSize, FACT_FLAG_MANAGEDATA, 0, &m_pSoundBank ) ) )
	{
		app.FatalLoadError();
		assert(false);
		return;
...
#ifdef _TU_BUILD
	file = CreateFile("Common/res/audio/additional.xsb", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
	file = CreateFile("Common/res/TitleUpdate/audio/additional.xsb", GENERIC_READ, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
	if( file == INVALID_HANDLE_VALUE )
	{
		app.FatalLoadError();
	...
	dwFileSize = GetFileSize(file,NULL);
	void *pvSoundBank2 = malloc(dwFileSize);
	ReadFile(file,pvSoundBank2,dwFileSize,&bytesRead,NULL);
	CloseHandle(file);

	if ( FAILED( hr = FACTAudioEngine_CreateSoundBank( m_pXACT3Engine, pvSoundBank2, dwFileSize, FACT_FLAG_MANAGEDATA, 0, &m_pSoundBank2 ) ) )
	{
		app.FatalLoadError();
		assert(false);
		return;
	...
	desc.pvContext=this;
	FACTAudioEngine_RegisterNotification(m_pXACT3Engine, &desc);

	// get the category to manage the sfx (Default)
	m_xactSFX = FACTAudioEngine_GetCategory(m_pXACT3Engine, "Default");
	m_xactMusic = FACTAudioEngine_GetCategory(m_pXACT3Engine, "Music");
}

void SoundEngine::CreateStreamingWavebank(const char *pchName, FACTWaveBank **ppStreamedWaveBank)
{
	// Create streamed sound bank
	HRESULT hr;

	FAudioIOStream *faudioFile = FAudio_fopen(pchName);
	...
	if ( FAILED( hr = FACTAudioEngine_CreateStreamingWaveBank( m_pXACT3Engine, &streamParams, ppStreamedWaveBank ) ) )
	{
		app.FatalLoadError();
		assert(false);
		return;
...
void SoundEngine::CreateSoundbank(const char *pchName, FACTSoundBank **ppSoundBank)
{
	HRESULT hr;
	...
	if ( FAILED( hr = FACTAudioEngine_CreateSoundBank( m_pXACT3Engine, pvSoundBank, dwFileSize, FACT_FLAG_MANAGEDATA, 0, ppSoundBank ) ) )
	{
		app.FatalLoadError();
		assert(false);
		return;
...
bool SoundEngine::isStreamingWavebankReady()
{
	...
	{
		DWORD dwState;
		FACTSoundBank_GetState(m_pSoundBank, &dwState);
		if(dwState&FACT_STATE_PREPARED)
		{
			m_bStreamingWaveBank1Ready=true;
		}
		FACTSoundBank_GetState(m_pSoundBank2, &dwState);
		if(dwState&FACT_STATE_PREPARED)
		{
...
bool SoundEngine::isStreamingWavebankReady(FACTWaveBank *pWaveBank)
{
	DWORD dwState;
	FACTWaveBank_GetState(pWaveBank, &dwState);
	if(dwState&FACT_STATE_PREPARED)
	{
		return true;
	}
...
void SoundEngine::XACTNotificationCallback( const FACTNotification* pNotification )
{
	if(pNotification->pvContext!= NULL)
	{
		if(pNotification->type==FACTNOTIFICATIONTYPE_WAVEBANKPREPARED)
		{
...
void SoundEngine::play(int iSound, float x, float y, float z, float volume, float pitch)
{
	...
	if(bSoundbank1)
	{
		idx = FACTSoundBank_GetCueIndex(m_pSoundBank, xboxName);
	}
	else
	{
	...
	uint16_t vidx = FACTCue_GetVariableIndex(cueInstance, "Pitch");
	if( vidx != FACTVARIABLEINDEX_INVALID )
	{
		// Convert pitch multiplier to semitones
		float semiTones = (log(pitch)/log(2.0f)) * 12.0f;
		FACTCue_SetVariable( cueInstance, vidx, semiTones );
	}
...
{
	FACTSoundBank *pSoundBank=NULL;
	
	bool bSoundBank2=false;
	MemSect(34);
	if(m_MusicInfo.pCue!=NULL)
	{
		FACTCue_Stop(m_MusicInfo.pCue, 0);
		FACTCue_Destroy(m_MusicInfo.pCue);
	...
	if(name.empty())
	{
		...
		if(playerInEnd)
		{
			m_musicIDX = FACTSoundBank_GetCueIndex(pSoundBank, "the_end_dragon");
			SetIsPlayingEndMusic(true);
			bSoundBank2=true;
		}
		else if(playerInNether)
		{
			m_musicIDX = FACTSoundBank_GetCueIndex(pSoundBank, "nether");
			SetIsPlayingNetherMusic(true);
			bSoundBank2=true;
		}
		else
		{
			m_musicIDX = FACTSoundBank_GetCueIndex(pSoundBank, "music");
		}
	}
	else
	{
		...
		m_musicIDX = FACTSoundBank_GetCueIndex(pSoundBank, ConvertSoundPathToName(name));
	}

	HRESULT hr;

	if( FAILED( hr = FACTSoundBank_Prepare(pSoundBank, m_musicIDX, 0, 0, &m_MusicInfo.pCue ) ) )
	{
		//		printf("Sound prep failed\n");
		m_musicIDX = FACTINDEX_INVALID; // don't do anything in the tick
		m_MusicInfo.pCue=NULL;
		MemSect(0);
		return;
	}
...
void SoundEngine::playMusicTick()
{
	...
	if( m_musicIDX == FACTINDEX_INVALID )
	{
	...
	if(m_MusicInfo.pCue!=NULL)
	{
		if( FAILED( hr = FACTCue_GetState(m_MusicInfo.pCue, &state) ) )
		{
			assert(false);
		}
		else
...
{
	float xactVol=fVal;
	HRESULT hr=FACTAudioEngine_SetVolume(m_pXACT3Engine, m_xactMusic,fVal);
}
...
{
	float xactVol=fVal;
	HRESULT hr=FACTAudioEngine_SetVolume(m_pXACT3Engine, m_xactSFX,fVal);
}

void SoundEngine::update3DPosition(SoundEngine::soundInfo *pInfo, bool bPlaceEmitterAtListener,bool bIsCDMusic)
{
	F3DAUDIO_LISTENER *listener = &m_listeners[0];	// Default case for single listener
	...
	FACT3DCalculate( m_xact3dInstance, listener, &m_emitter, &m_DSPSettings );
	...
	FACT3DApply( &m_DSPSettings, pInfo->pCue);
}
...
void SoundEngine::tick(std::shared_ptr<Mob> *players, float a)
{
	...
	for( unsigned int i = 0; i < currentSounds.size(); i++ )
	{
		...
		DWORD state;
		HRESULT hr;
		if( FAILED( hr = FACTCue_GetState(info->pCue, &state) ) )
		{
			assert(false);
		}
		else
	...
	LeaveCriticalSection(&m_CS);
	FACTAudioEngine_DoWork(m_pXACT3Engine);
}
